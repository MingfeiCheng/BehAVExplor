...
class NpcVehicle(Vehicle):
    ...
    def follow_waypoints_offset(self, waypoints, lans_to_follow):
        self.remote.command(
            "vehicle/follow_waypoints_offset",
            {
                "uid": self.uid,
                "waypoints": [
                    {
                        "lane_id": wp['lane_id'],
                        "start": wp['start'].to_json(),
                        "end": wp['end'].to_json(),
                        "offset": wp['offset'],
                        "speed": wp['speed'],
                    }
                ...
                "lanes_to_follow": [
                    {
                        "lane_id": lf['lane_id'],
                        "start": lf['start'].to_json(),
                        "end": lf['end'].to_json(),
                    }
                    for lf in lans_to_follow
                ]
            }
        )
    
    def follow_lanes_init(self, lanes_pool, aggression = 0):
        """
        var uid = args["uid"].Value;
            var lanesSequence = args["lanes_pool"].AsArray;
            var aggression = args["aggression"];
        """
        self.remote.command(
            "vehicle/follow_lanes_init",
            {
                "uid": self.uid,
                "lanes_pool": [
                    {
                    ...
                    for lf in lanes_pool
                ],
                "aggression": aggression
            }
        )
    
    def follow_lanes_start(self, speed):
        
        self.remote.command(
            "vehicle/follow_lanes_start",
            {
                "uid": self.uid,
                "speed": speed
            }
    ...
    def follow_lanes_change_lane(self, isLeftChange=True):
        """
        var uid = args["uid"].Value;
            var isLeft = args["isLeftChange"].AsBool;
        """
        print('PythonAPI: ' + str(isLeftChange))
        self.remote.command(
            "vehicle/follow_lanes_change_lane",
            {
                "uid": self.uid,
                "isLaneChange": isLeftChange,
            }
        )
    
    def follow_lanes_set_speed(self, speed):
        self.remote.command(
            "vehicle/follow_lanes_set_speed",
            {
                "uid": self.uid,
                "speed": speed,
            }
