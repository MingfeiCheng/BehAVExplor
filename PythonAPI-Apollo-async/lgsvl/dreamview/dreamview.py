...
class Connection:
    ...
    def enable_apollo(self, dest_x, dest_z, modules, coord_type=CoordType.Unity):
        ...
        """
        for mod in modules:
            logger.info("Starting {} module...".format(mod))
    ...
    def setup_apollo(self, dest_x, dest_z, modules, default_timeout=60.0, coord_type=CoordType.Unity):
        ...
        try:
            ...
            logger.warning("Invalid LGSVL__DREAMVIEW__CONTROL_MESSAGE_TIMEOUT_SECS, using default {0}s".format(default_timeout))
        ...
        while timeout <= 0.0 or float(elapsed) < timeout:
            ...
            if elapsed > 0 and elapsed % (run_time * 5) == 0:
                ...
                logger.info("{} seconds have passed but Ego hasn't received any control messages.".format(elapsed))
                logger.info("Please also check if your route has been set correctly in Dreamview.")

            elapsed += run_time
        else:
            logger.error("No control message from Apollo within {} seconds. Aborting...".format(timeout))
            self.disable_apollo()
